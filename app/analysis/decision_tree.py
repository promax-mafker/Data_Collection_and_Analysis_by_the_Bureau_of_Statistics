...
# 常住人口在 econ_series 的 note 标识（实测形如「常住人口（万人）」）
_POP_MARKERS = ("常住人口", "年末常住")


def _econ_by_note_markers(repo, indicator, markers):
    """econ_series 中 note 命中任一标识的行 → ``{year: float}``（无则空）。

    用于维度混装的 indicator：`population` 下同时装着常住人口/出生率/死亡率/
    自然增长率/人口密度/城镇化率，子维度只在 `note` 里 —— 必须先按 note 辨识。
    """
    out = {}
    for r in repo.list_econ_series(indicator=indicator):
        note = r.get("note") or ""
        if not any(m in note for m in markers):
            continue
        try:
            v = float(str(r.get("value", "")).replace(",", "").replace(" ", ""))
        except ValueError:
...
# CPI 在 econ_series 的 note 标识（实测 note 为 全国/全省/全市，2020 卷为「泉州」）
_CPI_MARKERS = ("全市", "泉州")


def _cpi_series(repo):
    """本地 CPI 序列 ``{year: 指数}``（仅取本地口径）。

    ⚠ `indicator='cpi'` 混装了**全国/全省/全市**三个地区（子维度只在 `note`），
    真实库 2024 年三值为 `100.2 / 99.9 / 100.0`。原实现
    `_econ_map(repo, "cpi")` 用字典覆盖 → 取到该年**最后一行**，即"把哪个地区的
    CPI 当本地"取决于行序（与已修的 population 同类缺陷）。
    按 P1「错 > 缺」：只认本地标识，认不出返回空。
    """
    return _econ_by_note_markers(repo, "cpi", _CPI_MARKERS)


def _pop_pairs(repo, region):
    """常住人口序列：优先 final data_values，其次 econ_series(population)。

    ⚠ 回退路径**必须按 note 辨识**「常住人口」：该 indicator 混装了 6 个指标，
    原实现用 `{year: value}` 字典覆盖会取到该年**最后一行** ——
    实测会把「人口密度 789」或「城镇化率 71.19」当成长住人口。
    按 P1「错 > 缺」：认不出就返回空，不拿别的指标冒充。
    """
    pairs = _final_pairs(repo, "常住人口", region)
    if pairs:
        return pairs
    return sorted(_econ_by_note_markers(repo, "population", _POP_MARKERS).items())
...
def source_independence_node(repo, region="泉州市"):
    """T5-1 跨源一致性（M9d）：关键指标是否有**独立口径**第二来源。

    设计 P5：同源搬运不算互证（公报与年鉴同属统计局）。因此本节点衡量的不是
    「来源数量」，而是「结论是否有独立证据」。只有单一来源时判 ``verify`` ——
    数据可能准确，但**我们没有独立证据证明它准确**，报告引用须标注。
    """
    from .crosscheck import cross_source_status, summarize_cross_source

    s = summarize_cross_source(cross_source_status(repo, region))
    if s["ok"] + s["single_source"] == 0:
        return _mk("T5-3", "T5", "关键指标是否具备独立第二来源?", "na",
                   "无关键指标数据", "需先补齐关键指标", "P5")
    ev = f"独立口径 {s['ok']} 项 / 单一来源 {s['single_source']} 项"
    if s["single_source"] == 0:
        return _mk("T5-3", "T5", "关键指标是否具备独立第二来源?", "ok", ev,
                   "全部关键指标均有独立口径互证", "P5")
    return _mk("T5-3", "T5", "关键指标是否具备独立第二来源?", "verify", ev,
               "单一来源:" + "、".join(s["single_list"])
               + " —— 无独立证据支撑,引用须标注来源单一", "P5")


def debt_nodes(debt, land_reliance=None):
    """债务专题决策节点（M9e）：T3-6 债务率 / T3-7 结构 / T3-8 效率 / T3-9 联动。

    设计要点：**缺数据一律 ``na`` 并写明缺什么** —— 报告里要能看出"为什么没结论"，
    而不是留一个空节点让人以为查过了。
    """
    balance = debt.get("balance") or {}
    limit = debt.get("limit") or {}
    own = debt.get("own_revenue") or {}
    ys = sorted(balance)
    latest = ys[-1] if ys else None

    # T3-6 债务率 = 债务余额 / 综合财力
    # ⚠ 口径警告：标准「债务率」的分母=综合财力，含**上级补助收入**（转移支付），
    #   后者在我国地市级财政中往往与自有财力同量级。本库目前只有自有财力
    #   （一般公共预算收入 + 政府性基金收入），故算出的比率是**上界**，
    #   一律标 verify 并写明口径，避免被当成可比指标引用（P1「错 > 缺」）。
    rev = own.get(latest) if latest else None
    if latest and rev:
        ratio = balance[latest] / rev * 100
        ev = (f"债务率(自有财力口径)={_num_fmt(ratio, 1)}%(债务{_num_fmt(balance[latest], 2)}亿元"
              f"/{latest}年自有财力{_num_fmt(rev, 2)}亿元)")
        n6 = _mk("T3-6", "T3", "债务率是否越过警戒区间?", "verify", ev,
                 f"自有财力口径 {_num_fmt(ratio,1)}%；未含上级补助收入，是标准债务率的**上界**，"
                 "需补转移支付收入后方可与他地比较", "P-debt-2")
    else:
        n6 = _mk("T3-6", "T3", "债务率是否越过警戒区间?", "na",
                 "缺综合财力（一般公共预算收入 + 政府性基金收入）",
                 "需同年财政两项收入方可计算债务率", "P-debt-2")

    # T3-7 一般债/专项债结构（需 CELMA 省级分项）
    n7 = _mk("T3-7", "T3", "一般债/专项债结构是否失衡?", "na",
             "缺一般债/专项债分项数据",
             "需接 CELMA 地方政府债券平台（省级分项，实测无鉴权可用）", "P-debt-5")

    # T3-8 新增债务的投资转化效率 = Δ债务 / Δ固定资产投资
    di = debt.get("delta_investment")
    if len(ys) >= 2 and di:
        dn = balance[ys[-1]] - balance[ys[-2]]
        eff = dn / di if di else None
        ev = f"Δ债务={_num_fmt(dn,2)}亿元/Δ固投={_num_fmt(di,2)}亿元"
        if eff is None:
            n8 = _mk("T3-8", "T3", "新增债务的投资转化效率是否衰减?", "verify", ev,
                     "固投增量为 0，无法计算转化效率", "P-debt-3")
        elif eff > 1.0:
            n8 = _mk("T3-8", "T3", "新增债务的投资转化效率是否衰减?", "verify", ev,
                     f"每 1 元固投增量对应 {_num_fmt(eff,2)} 元新增债务：举债拉动效率偏低", "P-debt-3")
        else:
            n8 = _mk("T3-8", "T3", "新增债务的投资转化效率是否衰减?", "info", ev,
                     f"每 1 元固投增量对应 {_num_fmt(eff,2)} 元新增债务", "P-debt-3")
    else:
        n8 = _mk("T3-8", "T3", "新增债务的投资转化效率是否衰减?", "na",
                 "缺固定资产投资增量或相邻两年债务余额",
                 "需同年固投增量与债务增量", "P-debt-3")

    # T3-9 土地依赖 × 债务空间联动
    common = [y for y in balance if y in limit and limit[y]]
    util = (balance[max(common)] / limit[max(common)] * 100) if common else None
    if land_reliance is None or util is None:
        n9 = _mk("T3-9", "T3", "土地依赖 × 债务空间是否联动放大风险?", "na",
                 "缺土地依赖或债务/限额数据",
                 "需土地出让收入占比与限额利用率", "P-debt-4")
    elif land_reliance > 30 and util > 90:
        n9 = _mk("T3-9", "T3", "土地依赖 × 债务空间是否联动放大风险?", "flag",
                 f"土地依赖={_num_fmt(land_reliance,1)}%;限额利用率={_num_fmt(util,1)}%",
                 "土地出让收入下滑与债务到期的联动风险放大", "P-debt-4")
    else:
        n9 = _mk("T3-9", "T3", "土地依赖 × 债务空间是否联动放大风险?", "ok",
                 f"土地依赖={_num_fmt(land_reliance,1)}%;限额利用率={_num_fmt(util,1)}%",
                 "两项均未同时进入高风险区", "P-debt-4")
    return [n6, n7, n8, n9]
...
def tree_audit(repo, region="泉州市") -> dict:
    ...
    # T3-6 ~ T3-9 债务专题（M9e，用户指定加权）
    # 复用同一份债务序列；缺数据的节点会以 na + 明确原因呈现（不静默省略）
    try:
        from .debt import debt_profile as _debt_profile
        nodes.extend(debt_nodes(_debt_profile(repo, region)))
        nodes.append(source_independence_node(repo, region))
    except Exception as e:
        nodes.append(_mk("T3-6", "T3", "债务专题节点是否可用?", "verify",
                         f"债务画像计算失败: {type(e).__name__}",
                         "债务序列读取异常，需检查 econ_series", "P-debt-1"))
